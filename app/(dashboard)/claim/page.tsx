...
import {
  ...
  Edit2, Search, RefreshCw, Settings2, Database,
   ExternalLink, ArrowUpDown, ChevronDown, ChevronUp,
  Plus, Trash2,
} from "lucide-react";
...
/**
 * Satu entri konfigurasi G-Sheet per expedisi, seperti dikembalikan
 * /api/claim/config. Kuncinya tetap KODE expedisi seperti dulu.
 *
 * `expedisiId` / `expedisiName` hanya tautan opsional ke master ekspedisi —
 * diisi server kalau kodenya kebetulan terdaftar, null kalau tidak. Halaman
 * ini tidak memakainya: penyimpanan dilakukan per kode.
 */
interface ClaimExpedisiSheet {
...
const EXP_COLORS: Record<string, string> = {
  ...
  TKP:       "bg-brand-100 text-brand-800",
...
const pesanError = (e: unknown) => (e instanceof Error ? e.message : String(e));
...
function Isi() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch("/api/claim/config", { cache: "no-store" });
        const d = (await r.json()) as Partial<ClaimSheetConfig> & { error?: string };
        if (!r.ok) throw new Error(d.error || "Gagal memuat konfigurasi claim.");
        ...
        setConfigError(pesanError(e));
      }
    })();
  }, []);

  /** PATCH /api/claim/config selalu mengembalikan konfigurasi terbaru. */
  const patchConfig = useCallback(
    async (body: Record<string, unknown>): Promise<ClaimSheetConfig> => {
      const r = await fetch("/api/claim/config", {
        method:  "PATCH",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify(body),
      });
      const d = (await r.json()) as Partial<ClaimSheetConfig> & { error?: string };
      if (!r.ok) throw new Error(d.error || "Gagal menyimpan konfigurasi claim.");
      return {
        masterSpreadsheetId: d.masterSpreadsheetId ?? "",
        expedisiSheets:      d.expedisiSheets ?? {},
      };
  ...
  const handleSaveMaster = async (id: string) => {
    const next = await patchConfig({ masterSpreadsheetId: id.trim() });
    setConfig(next);
  };

  /**
   * Sheet yang dibuat otomatis oleh /api/claim/upload — simpan semuanya
   * sekaligus dalam satu PATCH, lalu pakai respons PATCH sebagai state baru.
   */
  const handleNewSheets = async (
    newSheets: Record<string, { spreadsheetId: string; url: string }>
  ) => {
    const sheets = Object.entries(newSheets).map(([code, s]) => ({
      code,
      spreadsheetId: s.spreadsheetId,
      url:           s.url,
    }));
    if (sheets.length === 0) return;
    const next = await patchConfig({ sheets });
    setConfig(next);
  };
  ...
  const handleSaveExpedisiId = async (code: string, spreadsheetId: string) => {
    ...
    const next = await patchConfig({
      sheet: { code: code.trim().toUpperCase(), spreadsheetId: id, url: GSHEET_URL(id) },
    });
    setConfig(next);
  };
    ...
    <div className="max-w-full mx-auto space-y-5">
      ...
      {configError && (
        <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 flex items-center gap-3 text-sm text-red-700">
          ...
          <button onClick={() => setConfigError("")}><X className="w-4 h-4" /></button>
        </div>
      )}
  ...
  return (
    <div className="card p-5 space-y-5">
      <p className="font-semibold text-ink flex items-center gap-2 text-sm">
        <Settings2 className="w-4 h-4 text-gray-400" /> Konfigurasi Google Sheets
      </p>
      ...
      <div>
        <label className="text-xs text-gray-500 mb-1.5 block">
          Master Spreadsheet{" "}
          <span className="text-gray-400">(semua data — tab ALL)</span>
        </label>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-xs text-gray-500">
            ...
            <span className="text-gray-400">(1 file G-Sheet per expedisi)</span>
          </label>
          <button
                    ...
                    <td className="px-3 py-2">
                      <div className="flex flex-wrap gap-1">
                        {getPrefixesForExpedisi(code).map((p) => (
                          <span key={p} className="px-1.5 py-0.5 bg-gray-100 text-gray-500 rounded text-xs font-mono">
                            {p}
                          </span>
                        ))}
                    ...
                    <td className="px-3 py-2 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button
                          ...
                          className="p-1.5 rounded text-brand-600 hover:bg-brand-100 disabled:opacity-40"
                        >
                          <Check className="w-3.5 h-3.5" />
                        </button>
                        <button
  ...
  return (
    <div className="space-y-5">
      ...
      <label
        className={cn(
          ...
          file
            ...
            : "border-gray-300 hover:border-brand-200 hover:bg-gray-50"
        )}
      ...
      >
        ...
        />
        {file
          ? <FileSpreadsheet className="w-10 h-10 mx-auto mb-3 text-brand-600" />
        ...
        {file ? (
          ...
          <>
            <p className="font-semibold text-gray-600">Klik atau drag file Excel di sini</p>
            <p className="text-sm text-gray-400 mt-1">Format: .xlsx atau .xls (ekspor dari Jubelio)</p>
          </>
        )}
      </label>
      ...
      {result && (
        <div className="bg-brand-50 border border-brand-200 rounded-xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            {[
              { val: result.added,   label: "Baris ditambahkan",    cls: "text-brand-600" },
              { val: result.skipped, label: "Sudah ada (dilewati)", cls: "text-gray-400" },
              { val: result.total,   label: "Total baris di file",   cls: "text-gray-600" },
            ].map(({ val, label, cls }) => (
          ...
          {Object.keys(result.expedisiSummary).length > 0 && (
            <div>
              <p className="text-xs text-brand-700 font-medium mb-1.5">Baris baru per expedisi:</p>
              ...
              <div className="space-y-1">
                {Object.entries(result.newSheets).map(([code, s]) => (
                  <div key={code} className="flex items-center gap-2 text-xs">
                    ...
                    <span className="font-mono text-gray-500 truncate flex-1">{s.spreadsheetId}</span>
...
function EditTab({ config }: { config: ClaimSheetConfig }) {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      setEditCell(null);
    } catch (e) { setSaveError(pesanError(e)); }
  ...
  const handleDelete = async (idx: number) => {
    ...
    try {
      ...
      if (!res.ok) {
        const d = await res.json();
        throw new Error(d.error || "Gagal menghapus baris");
      }
      setRows((prev) =>
        prev.filter((_, i) => i !== idx).map((r, i) => ({ ...r, no: String(i + 1) }))
      );
      setDelConfirm(null);
    } catch (e) { setDelError(pesanError(e)); }
  ...
  if (sources.length === 0) {
    return (
      <div className="card p-12 text-center">
        ...
        <p className="font-semibold text-gray-500">Konfigurasi Spreadsheet ID terlebih dahulu</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-4 space-y-3">
        {/* Source selector */}
        <div>
          <p className="text-xs text-gray-500 mb-2">Pilih Sheet</p>
          <div className="flex flex-wrap gap-2">
            {sources.map((s, i) => (
              <button
                key={i}
            ...
            {currentSource && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="px-3 py-1.5 rounded-lg text-xs font-medium border border-gray-300 text-gray-400 hover:text-brand-600 hover:border-brand-200 flex items-center gap-1"
              >
      ...
      {rows.length > 0 && (
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-sm font-semibold text-ink">{rows.length} baris</span>
      ...
      {loading ? (
        <div className="card flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm text-gray-400">Mengambil data dari Google Sheets...</p>
        </div>
      ) : rows.length > 0 ? (
        <div className="card overflow-hidden">
              ...
              <thead className="thead-ocs">
                <tr>
                  ...
                  <th className="px-3 py-3 text-center w-36">
                    <button
                      onClick={() => setSortDir((d) => d === "desc" ? "asc" : "desc")}
                      className="flex items-center gap-1 mx-auto hover:text-brand-800"
                    >
                      Created Date
                      {sortDir === "desc"
              ...
              <tbody className="divide-y divide-gray-200">
                {rows.map((row, idx) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-3 py-2 text-center text-gray-400">{row.no || idx + 1}</td>
                            ...
                            {isEditing ? (
                              <div className="flex items-center gap-1">
                                <input
                                  ...
                                  className="border border-brand-400 rounded px-2 py-0.5 text-xs focus:outline-none focus:ring-2 focus:ring-brand-400 w-28 font-mono"
                                />
                                <button
                                  onClick={saveEdit}
                                  disabled={saving}
                              ...
                              <span
                                className={cn(
                                  ...
                                  mono && "font-mono font-semibold text-heading",
                                  !mono && "text-gray-600"
                                )}
          ...
          <div className="px-4 py-2.5 border-t border-gray-200 bg-gray-50 flex justify-between text-xs text-gray-400">
        ...
        <div className="card p-16 text-center">
          ...
          <p className="font-semibold text-gray-500">Klik Tampilkan untuk muat data</p>
        </div>
      ) : null}
    </div>
  );
